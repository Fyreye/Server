...
// Similar to GL_example3, but applies the transformation
// using a matrix in the vertex shader.  See the animate loop
// for various kinds of rotations.  
//
...
function draw(modelMatrix, modelMatrix1)
{
...
function main() {
  ...
  {
    draw(modelMatrix1, modelMatrix2);

    degrees += (increment/2);
	degrees2 += (increment);
	
    // new position using a translation
    var x = .75 * Math.cos(degrees * Math.PI / 180.0);
    var y = .75 * Math.sin(degrees * Math.PI / 180);
    modelMatrix1.setTranslate(x, y, 0);
	modelMatrix1.multiply(new Matrix4().setRotate(degrees2, 0, 0, 1));
	modelMatrix1.multiply(new Matrix4().setScale(0.15,0.4,1));
	
    modelMatrix2.setTranslate(x, y, 0);
	modelMatrix2.multiply(new Matrix4().setRotate(degrees2, 0, 0, 1));
	modelMatrix2.multiply(new Matrix4().setScale(0.4,0.15,1));
