...
function draw()
{
  ...
  var shift_loc = gl.getUniformLocation(shaderSolid, "shift");
  gl.uniform1f(shift_loc, -.5);
  
  var color_loc = gl.getUniformLocation(shaderSolid, "colorVar");
  
  // bind the buffer
  gl.bindBuffer(gl.ARRAY_BUFFER, vertexbuffer);

  // get the index for the a_Position attribute defined in the vertex shader
  var positionIndex = gl.getAttribLocation(shaderSolid, 'a_Position');
  if (positionIndex < 0) {
  ...
  gl.useProgram(shaderSpec);
  
  // update the uniform variable 'shift'  and draw again. 
  // Note the data buffer is still associated with the a_Position vertex attribute
  shift_loc = gl.getUniformLocation(shaderSpec, "shift");
  gl.uniform1f(shift_loc, .5);
  gl.drawArrays(gl.TRIANGLES, 0, numPoints);
...
function main() {
  ...
  var vshaderSource = document.getElementById('vertexShaderSolid').textContent;
  var fshaderSource = document.getElementById('fragmentShaderSolid').textContent;
  ...
  shaderSolid = gl.program;
  gl.useProgram(null);
  
  
  //bind second shader pair
  var vshaderSource = document.getElementById('vertexShaderSpecular').textContent;
  fshaderSource = document.getElementById('fragmentShaderSpecular').textContent;
  if (!initShaders(gl, vshaderSource, fshaderSource)) {
    console.log('Failed to intialize shaders.');
    return;
  }
  
  shaderSpec = gl.program;
  gl.useProgram(null);
